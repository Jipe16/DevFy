...
const HomeIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L1 12h3v10h14V12h3L12 2zm0 2.83L17.17 10H6.83L12 4.83zM17 19H7v-7h10v7z" /></svg>
);
...
const recommendationsData: Recommendation[] = [
  ...
  { id: 3, type: 'video', source: './assets/video3.mp4', poster: './assets/gorillaz_cover.jpg', title: 'Presenting Gorillaz', description: 'Gorillaz, Miho Hatori' },
  { id: 4, type: 'image', source: './assets/relaxing_pop_cover.jpg', title: 'Relaxing Soft Pop', description: 'Ed Sheeran, James Arthur, Coldplay, Sam Smith' },
];
...
const Sidebar = () => (
  <aside css={styles.sidebar}>
    <div css={styles.sidebarSection}>
        <Link href={`/home_screen`} passHref css={styles.sidebarLink} className="active">
            <HomeIcon /> HOME
        </Link>
    ...
    <div css={styles.sidebarSection}>
      ...
      <div css={styles.playlistsSection}>
        <p>PLAYLISTS</p>
        <button css={styles.createPlaylistBtn}><PlusIcon /> CREATE PLAYLIST</button>
...
const RecommendationCard = ({ item }: { item: Recommendation }) => {
    const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (video) video.play().catch(error => console.log("Video autoplay failed:", error));
    };
    ...
    return (
        <div css={styles.card} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <div css={styles.cardMedia}>
                {item.type === 'video' ? (
                    <video src={item.source} poster={item.poster} muted loop playsInline css={styles.mediaContent}/>
                ) : (
